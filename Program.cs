 ...
 void opcoesMenu() //Função que vai abrir o menu e passar as condições.
{
    ...
    switch (opcaoUsuarioNumber)
    {
        ...
        case 2:
            registroHistoricoBandas(); //chama a função de 
            break;
        ...
        case -1:
            Console.WriteLine("A opção selecionada por você foi a opção " + opcaoUsuarioNumber + " - Sair do sistema");
            Console.WriteLine("\nAperte qualquer tecla para fechar o programa... Obrigado.");
            break;
    ...
    void regisrarBanda(){ //Função que registra as bandas :
    ...
    void mediaAvaliacao()
    {
        ...
        if (historicoBandas.ContainsKey(nomeDaBanda))
        {
            ...
            Console.ReadKey(); //Entende que q alguma tecla foi pressionada
            Console.Clear(); //limpa o console
            opcoesMenu(); //volta ao menu principa
        } else
        {
            Console.WriteLine($"A banda {nomeDaBanda} não foi encontrada/não foi registrada!");
            Console.WriteLine("\n\nTecle qualquer coisa para retornar ao menu");
